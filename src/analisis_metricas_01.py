...
# Redondeo las columnas 'mean' y 'std' a 2 decimales
...
# Renombro la columna 'mean' como 'promedio'
df_grouped.rename(columns={'mean': 'promedio'}, inplace=True)

# Renombro la columna '50%' como 'mediana' en minúsculas
df_grouped.rename(columns={'50%': 'mediana'}, inplace=True)

# Renombro la columna 'count' como 'cantidad'
df_grouped.rename(columns={'count': 'cantidad'}, inplace=True)

# Veo en la IDE del Spyder
print(df_grouped)

# Guardo los descriptores estadísticos en un archivo CSV en la raíz del repo
